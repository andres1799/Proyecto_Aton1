...
$sql = $con->prepare("SELECT PR.idproducto, PR.nombre, PR.descripcion, PR.precio, PR.idcategoria, PR.imagen, CT.categoria FROM productos as PR INNER JOIN categorias as CT ON CT.idcategoria = PR.idcategoria WHERE activo = 1 AND PR.idcategoria = 2 LIMIT  5 ");
$sql->execute();
        ...
        <div class="col-md-3">
            <main class="grid">
                <article>
                    <?php
                    $id = $row['idproducto'];
                    $dir = "../assets/img/Monitores/" .$id;
                    /*if(!is_dir($dir)){
                        mkdir($dir);
                    }*/
                    $imagen = "../assets/img/" .$row['categoria']."/".$row['idproducto']."/".$row['imagen'];
                    if (!file_exists($imagen)) {
                        $imagen = "http://localhost:8080/Aton_1_7/assets/img/no-photo.jpg";
                    }
 
                    ?>
                    <img src="<?php echo $imagen; ?>" width="100" alt="" class="img-thumbnail">
                    <div class="card-body">
                        <h3><?php echo $row['nombre']; ?></h3>
                        <p class="card-text"><?php echo number_format($row['precio'], 2, '.', ','); ?></p>
                        <p>dolor sit amet consectetur adipisicing elit. Id eos, deserunt voluptatem,</p>
